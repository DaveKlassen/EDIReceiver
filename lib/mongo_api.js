...
var saveDataObject = function (ediDocument) {
  ...
  if ("mongo" === CONFIG.DB_IMPL) {
    ...
    MongoClient.connect(CONFIG.WEB_APP_DB, { useNewUrlParser: true }, function(err, db) {
      if (err) {
        lutils.multiLog("Connect Error - ID# " + ediDocument.id + " : " + err);
      } else {
        let dbase = db.db(CONFIG.WEB_APP);
        var collection = dbase.collection(CONSTANTS.SUBMITTED_EDI_COLLECTION);

        collection.insertOne(ediDocument, {w:1}, function(err2, result) {
          if (err2) {
